...
class EntityLifeCycleTest {
    ...
    void testDetachedEntity(int menuCode,int menuPrice){
        ...
        // detach : 특정 엔티티만 준영속 상태(관리하던 객체를 관리하지않는 상태로 변경) 로 만듦
    ...
    @DisplayName("준 영속화 detach 된 것 다시 merge로 영속화 테스트")
    @ParameterizedTest
    @CsvSource({"11,1000","12,1000"})
    void testDetachedMergeEntity(int menuCode,int menuPrice){
    ...
    @DisplayName("준 영속화 detach 된 것 다시 merge로 영속화 된 데이터 update 테스트")
    @ParameterizedTest
    @CsvSource({"11,민초민초","12,후루후루"})
    ...
    @DisplayName("준 영속화 detach 된 것 다시 merge로 영속화 된 데이터 save 테스트")
    @Test
    void testMergeSave(){
        ...
        Menu found = entityManager.find(Menu.class, 13);
        entityManager.detach(found);
        found.setMenuName("치약초코");
        found.setMenuCode(999);
        entityManager.merge(found);
        entityTransaction.commit();

        assertEquals("치약초코",entityManager.find(Menu.class,999).getMenuName());
    }
