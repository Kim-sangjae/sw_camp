...
// 클래스 문법과 유사하게 기능하는 것 처럼 보이는 생성자 함수와 비교
...
// 차이점 1. 클래스 생성자를 new와 함께 호출하지 않으면 에러가 발생하도록 함
// TypeError: Class constructor Student cannot be invoked without 'new'
// Student();
Teacher();  // 에러 발생하지 않고 일반 함수 호출로 동작함

// 차이점 2. 클래스에 정의 된 메서드는 열거 불가
for(let pro in student) {
    console.log(`반복문 student : ${pro}`);
}
for(let pro in teacher) {
    console.log(`반복문 teacher : ${pro}`);
}