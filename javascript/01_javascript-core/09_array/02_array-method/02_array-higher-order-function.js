// 배열 고차 함수 (고차함수 - 함수를 인수로 전달 받거나 함수를 반환하는 함수)
...
// sort의 기본 정렬은 문자 정렬이므로 필요하다면 정렬 기준 콜백 함수를 전달한다.
...
// Array.prototype.map : 배열의 요소 전체를 대상으로 콜백함수 호출 후
// 반환 된 값으로 구성 된 새로운 배열 반환
...
// Array.prototype.filter : 배열 요소 전체를 대상으로 콜백함수 호출 후 반환 값이 true인
// 요소로만 구성 된 새로운 배열 반환
...
// 그 결과를 boolean으로 반환
let result = [1, 5, 4, 3, 2].some(item => item > 10);
console.log(result);
result = [1, 5, 4, 3, 2].some(item => item > 3);
console.log(result);

// Array.prototype.every : 배열 내 모든 요소가 콜백 함수의 테스트를 통과하는지 확인하며
// 그 결과를 boolean으로 반환
result = [1, 5, 4, 3, 2].every(item => item > 3);
console.log(result);
result = [1, 5, 4, 3, 2].every(item => item > 0);
console.log(result);
...
// 그 결과가 참인 첫 번째 요소의 인덱스 반환한다. 참인 요소가 존재하지 않는다면 -1을 반환한다.
const students = [
    { name : '홍길동', score : 90 },
    { name : '유관순', score : 80 },
    { name : '장보고', score : 70 },
...
console.log(result);
// 만약 80점 이상인 학생들을 찾고 싶다면? find가 아닌 filter 사용
